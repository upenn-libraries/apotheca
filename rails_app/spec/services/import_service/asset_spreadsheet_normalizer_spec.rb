...
describe ImportService::AssetSpreadsheetNormalizer do
  let(:transformer) { described_class }
  let(:assets_data) do
    { spreadsheet: [{ filename: 'a.tif', sequence: '1', annotation: ['scribble'] },
  ...
  describe '.process' do
    ...
    it 'removes the spreadsheet field' do
      expect(transformed_data.key?(:spreadsheet)).to be false
    end
