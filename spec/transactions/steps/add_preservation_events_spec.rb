...
describe Steps::AddPreservationEvents do
  describe '#call' do
    ...
    context 'with a newly ingested Asset' do
      ...
      it 'sets a filename change event' do
        filename_events = result.value!.preservation_events.select { |ev| ev.event_type.to_s == Premis::Events::EDIT_FILENAME.uri }
        expect(filename_events.length).to eq 1
        expect(filename_events.first.outcome_detail_note).to eq(
          "File's original filename renamed from #{change_set.resource.original_filename} to #{change_set.preservation_file_id}"
        )
      end
    end
    ...
    context 'with an Asset receiving only updated metadata' do
      ...
      it 'does not set an filename event' do
        filename_event = result.value!.preservation_events.find { |ev| ev.event_type.to_s == Premis::Events::EDIT_FILENAME.uri }
        expect(filename_event).to be_nil
      end
    end
  end
end
