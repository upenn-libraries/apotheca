...
describe UpdateAsset do
  ...
  describe '#call' do
    ...
    let(:file1) do
      ActionDispatch::Http::UploadedFile.new tempfile: File.open(file_fixture('files/trade_card/front.tif')), filename: 'front.tif'
    end
    let(:file2) do
    ...
    context 'when preservation file does not have original filename' do
      ...
      let(:file1) { ActionDispatch::Http::UploadedFile.new tempfile: File.open(file_fixture('files/trade_card/front.tif')) }
      let(:asset) { persist(:asset_resource) }
      let(:result) do
