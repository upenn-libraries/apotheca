    ...
    respond_to do |format|
      if valid_drive? && valid_path?
        if filenames.present?
          ...
          format.csv {}
          format.json { render json: { filenames: 'That path has no files.', drive: params[:drive], path: params[:path] }, status: :ok }
          # format.json { render json: { filenames: 'That path has no files.' }, status: :unprocessable_entity }
        end
      else
        format.csv {}
